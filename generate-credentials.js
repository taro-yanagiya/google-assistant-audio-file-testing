...
if (process.argv.length <= 2) {
  console.error(
    "Please include the credentials downloaded from the Google Cloud Console"
  );
  console.error(
    "Then run `node generate-credentials.js /path/to/credentials.json`"
  );
  process.exit(1);
}
...
fs.readFile(clientSecretPath, fileOptions, (err, data) => {
  ...
  const scopes = ["https://www.googleapis.com/auth/assistant-sdk-prototype"];
